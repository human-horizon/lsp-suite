/**
 * swift-lsp - Swift Language Server using tree-sitter
 */
...
void extract_symbols(TSNode node, const TSLanguage *lang) {
    if (symbol_count >= 256) return;

    TSSymbol sym = ts_node_symbol(node);
    const char *name = ts_language_symbol_name(lang, sym);

    if (strcmp(name, "class_declaration") == 0 ||
        strcmp(name, "struct_declaration") == 0 ||
        strcmp(name, "func_declaration") == 0 ||
        strcmp(name, "enum_declaration") == 0 ||
        strcmp(name, "protocol_declaration") == 0) {
    ...
    for (unsigned int i = 0; i < child_count; i++) {
        extract_symbols(ts_node_child(node, i), lang);
    }
}
...
int main() {
    ...
    while (1) {
        ...
        if (strcmp(method, "initialize") == 0) {
            const char *result = "{\"capabilities\":{\"textDocument\":{\"documentSymbol\":{\"hierarchicalDocumentSymbolSupport\":true},\"hoverProvider\":true}},\"serverInfo\":{\"name\":\"swift-lsp\",\"version\":\"0.1.0\"}}";
            ...
            if (!source) {
                ...
                extract_symbols(root, tree_sitter_swift());
            ...
            send_response(id, "{\"contents\":{\"kind\":\"markdown\",\"value\":\"Swift symbol\"}}");
