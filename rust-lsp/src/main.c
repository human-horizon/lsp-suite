/**
 * rust-lsp - Rust Language Server using tree-sitter
 */
...
void extract_symbols(TSNode node, TSLanguage *lang) {
    if (symbol_count >= 256) return;

    TSSymbol sym = ts_node_symbol(node);
    const char *name = ts_language_symbol_name(lang, sym);

    if (strcmp(name, "function_item") == 0 ||
        strcmp(name, "struct_item") == 0 ||
        strcmp(name, "enum_item") == 0 ||
        strcmp(name, "trait_item") == 0 ||
        strcmp(name, "impl_item") == 0 ||
        strcmp(name, "macro_invocation") == 0) {
...
int main() {
    ...
    while (1) {
        ...
        if (strcmp(method, "initialize") == 0) {
            const char *result = "{\"capabilities\":{\"textDocument\":{\"documentSymbol\":{\"hierarchicalDocumentSymbolSupport\":true},\"hoverProvider\":true}},\"serverInfo\":{\"name\":\"rust-lsp\",\"version\":\"0.1.0\"}}";
            ...
            if (!source) {
                ...
                extract_symbols(root, tree_sitter_rust());
            ...
            send_response(id, "{\"contents\":{\"kind\":\"markdown\",\"value\":\"Rust symbol\"}}");
